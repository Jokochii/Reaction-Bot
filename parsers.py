...
async def extract_readybot_data(message):
    ...
    def get_filename(url_str):
        ...
        return url_str.split('?')[0].split('/')[-1].lower()
    ...
    if snapshot and not product_url:
        ...
        if getattr(snapshot, 'embeds', None):
    ...
    if item_title == "Unknown Item" and raw_content:
        ...
        for content in brackets:
            ...
            if clean_content.lower() in ['po', 'go', 'open', 'closed'] or re.search(r'ends|closing|deadline|\d{1,2}[/-]\d{1,2}', clean_content, flags=re.IGNORECASE):
                continue
...
def parse_date_from_text(text):
    ...
    for idx, word in enumerate(words):
        if word in months_map:
            ...
            if idx + 1 < len(words) and re.match(r'^(\d{1,2})(?:st|nd|rd|th)?$', words[idx + 1]): found_day = int(re.match(r'^(\d{1,2})', words[idx + 1]).group(1))
            if idx - 1 >= 0 and not found_day and re.match(r'^(\d{1,2})(?:st|nd|rd|th)?$', words[idx - 1]): found_day = int(re.match(r'^(\d{1,2})', words[idx - 1]).group(1))
