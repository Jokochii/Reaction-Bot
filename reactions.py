...
from parsers import parse_date_from_text, extract_readybot_data
    ...
    async def on_raw_reaction_add(payload):
        ...
        if len(COOLDOWNS[up_key]) >= 5:
            ...
            try:
                ...
                w_emb = discord.Embed(title="⛔ Anti-Spam Interaction Lockout",
                                      description=f"Hello {m.display_name}.\n\nYou spammed the reaction button for **{title}**.\n\nYour account is **locked out** from tracking **ALL posts** for the next **1 hour**.",
        ...
        try:
            ...
            if msg_key not in db:
                db[msg_key] = {"end_date": end_date_str, "channel_id": channel.id, "alerts_sent": False, "users": [],
                               "raw_images": image_list}
            ...
            if uid_str in db[msg_key]["users"]:
                ...
                opt_out = discord.Embed(title="❌ Reminder Cancelled",
                                        description=f"You have **opted out** of alerts for **{item_title}**.",
                                        color=0xe74c3c, timestamp=datetime.now())
                try:
                    await member.send(embed=opt_out)
                except Exception:
                    pass
                ...
                embed.set_footer(text="Automated Reminder Service",
                ...
                view.add_item(
                    discord.ui.Button(label="💬 Jump to Post", url=message.jump_url, style=discord.ButtonStyle.link))
                if product_url: view.add_item(
                    discord.ui.Button(label="🛒 Open Webpage", url=product_url, style=discord.ButtonStyle.link))
