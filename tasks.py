            ...
            for msg_id, payload in data.items():
                if "users" in payload and payload["users"]:
                    ...
                    for idx, img_url in enumerate(payload["raw_images"]):
                        if img_url and (
                                "://discordapp.com" in img_url or "media.discordapp.net" in img_url) and not img_url.startswith(
                            "local_"):
                            try:
    ...
    for msg_id in list(db.keys()):
        ...
        try:
            ...
            if (target_date - now).total_seconds() <= 86400 and not item_data.get("alerts_sent", False):
                ...
                embed = discord.Embed(title="⏰ Event Reminder Warning!",
                                      description="An event you track concludes within 24 hours.", color=0x3498db,
                                      timestamp=now, url=base_url)
                    ...
                    for idx, img_path in enumerate(img_list):
                        if img_path.startswith("local_"):
                            ...
                            if os.path.exists(true_path):
                                ...
                                if idx == 0:
                                    embed.set_image(url=f"attachment://{filename_raw}")
                                else:
                ...
                view.add_item(
                    discord.ui.Button(label="💬 Jump to Post", url=msg.jump_url, style=discord.ButtonStyle.link))
                if prod_url: view.add_item(
                    discord.ui.Button(label="🛒 Open Webpage", url=prod_url, style=discord.ButtonStyle.link))
